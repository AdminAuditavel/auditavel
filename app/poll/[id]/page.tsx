...
export default function PollPage() {
  ...
  async function sendVote(payload: Record<string, any>, setMsg: (s: string | null) => void, defaultErr: string) {
    ...
    if (!pid || !uh) {
      const msg = "Identidade do participante indisponível. Recarregue a página.";
      console.error(msg, { pid, uh, poll_id: safeId });
      setMsg(msg);
    ...
    if (!participantId) setParticipantId(pid);
    if (!userHash) setUserHash(uh);
    ...
    <main className="min-h-screen bg-surface">
      <div className="p-6 max-w-xl mx-auto space-y-5">
        ...
        <div className="text-center text-xs flex items-center justify-center gap-2 text-[color:var(--foreground-muted)]">
          <Image src="/Logo_A-removebg-preview.png" alt="Auditável" width={18} height={18} className="inline-block" />
